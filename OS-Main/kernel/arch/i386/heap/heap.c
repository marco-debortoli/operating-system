...
static uint32_t contract(uint32_t new_size, heap_t *heap)
{
	...
	// Can't contract past the HEAP_MIN_SIZE
...
static int32_t find_smallest_hole(uint32_t size, uint8_t page_align, heap_t *heap)
{
	...
	{
		header_t *header = (header_t *) lookup_ordered_map(iterator, &heap->index);

		// If the page-aligned memory
		if ( page_align > 0 )
		{
...
heap_t* create_heap(uint32_t start, uint32_t end, uint32_t max, uint8_t supervisor, uint8_t readonly)
{
	...
	// Start address and end address must be page aligned
	ASSERT(start % 0x1000 == 0 );
	ASSERT(end % 0x1000 == 0 );
	
	heap->index = place_ordered_map ( (void*) start, HEAP_INDEX_SIZE, (void*)header_t_less_than);
	...
	// Start off wih one large hole
	header_t *hole = (header_t*)start;
	hole->size = end - start;
	hole->magic = HEAP_MAGIC;
	hole->is_hole = 1;
	insert_ordered_map((void*)hole, &heap->index);
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
	...
	if ( iterator == -1 )
	{
		...
		while (iterator < heap->index.size)
		{
			uint32_t tmp = (uint32_t) lookup_ordered_map(iterator, &heap->index);
		...
		if (idx == -1)
		{
			...
			insert_ordered_map((void*)header, &heap->index);
		}
		else
		{
			// The last header needs adjusting.
			header_t *header = lookup_ordered_map(idx, &heap->index);
			header->size += new_length - old_length;
	...
	header_t *orig_hole_header = (header_t *)lookup_ordered_map(iterator, &heap->index);
	...
	else
	{
		...
		remove_ordered_map(iterator, &heap->index);
	}
	...
	if (orig_hole_size - new_size > 0)
	{
		...
		insert_ordered_map((void*)hole_header, &heap->index);
	}
...
void free(void *p, heap_t *heap)
{
	...
	if (test_header->magic == HEAP_MAGIC && test_header->is_hole)
	{
		...
		uint32_t iterator = 0;
		while ( (iterator < heap->index.size) && (lookup_ordered_map(iterator, &heap->index) != (void*)test_header) )
			iterator++;
